...
def generate_flashinfer_cu() -> list[str]:
    page_sizes = os.environ.get("LOQUETIER_PAGE_SIZES", "16").split(",")
    group_sizes = os.environ.get("LOQUETIER_GROUP_SIZES", "1,2,4,8").split(",")
    head_dims = os.environ.get("LOQUETIER_HEAD_DIMS", "64,128").split(",")
...
def get_version() -> str:
    version = os.getenv("LOQUETIER_BUILD_VERSION")
    if version is None:
...
def generate_build_meta() -> None:
    ...
    with open(root / "src/loquetier/_build_meta.py", "w") as f:
...
if __name__ == "__main__":
    ...
    ext_modules.append(
        torch_cpp_ext.CUDAExtension(
            name="loquetier.ops._kernels",
            sources=[
                "csrc/loquetier_ops.cc",
                "csrc/flashinfer_adapter/flashinfer_all.cu",
                "csrc/rms_norm/rms_norm_cutlass.cu",
                "csrc/smlm_forward/smlm_forward_cutlass.cu"
            ]
